...
namespace INVENTARIO_INSTITUCIONAL.Modelo
{
    ...
    {
        private string tabla = "monitor";
        private Conexion conexion;
        public MonitorModel()
        {
            conexion = new Conexion();
        }
        public int Create(EMonitor entidad)
        {
            try
            {
                string query = $"INSERT INTO {tabla} (cbi, descripcion, marca, modelo, serie, dimensiones, condicion, estado, aula_id, observaciones) " +
                               "VALUES (@cbi, @descripcion, @marca, @modelo, @serie, @dimensiones, @condicion, @estado, @aula_id, @observaciones)";
                ...
                cmd.Parameters.AddWithValue("@dimensiones", entidad.Dimensiones);
            ...
            {
                throw new Exception("Error al crear teclado: " + ex.Message);
            }
            finally
            {
                conexion.Close();
        ...
        public List<EMonitor> ReadAll()
        {
            List<EMonitor> list = new List<EMonitor>();
            ...
            while (reader.Read())
            {
                EMonitor monitor = new EMonitor
                {
                    ...
                    Dimensiones = reader.GetString("dimensiones"),
        ...
        public int Update(EMonitor entidad)
        {
            string query = $"UPDATE {tabla} SET cbi=@cbi, descripcion=@descripcion, marca=@marca, modelo=@modelo, " +
                           "serie=@serie, dimensiones=@dimensiones, condicion=@condicion, " +
