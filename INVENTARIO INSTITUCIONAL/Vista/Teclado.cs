...
namespace INVENTARIO_INSTITUCIONAL.Vista
{
    ...
    {
        private TecladoControlador control;
        private int id_selected = -1;
        private List<EAula> aulas;
        public Teclado()
        {
            InitializeComponent();
            control = new TecladoControlador();
            initdata();
            listarTodo();
            clean();
            cbo_aula_id.SelectedIndex = -1;
            dt_teclado.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;
        }
        ...
        private void listarTodo()
        {
            ...
            try
            {
                List<ETeclado> list = control.GetAll();
                foreach (ETeclado teclado in list)
                {
                    string aulaDesc = aulas
                        .Find(a => a.Id == teclado.Aula_id)?
                        .Descripcion_aula ?? "Sin aula";

                    dt.Rows.Add(teclado.Id, teclado.Cbi, teclado.Descripcion, teclado.Marca, teclado.Modelo,
                                teclado.Serie, teclado.Condicion, teclado.Estado, aulaDesc, teclado.Aula_id, teclado.Observaciones);
                }
            }
            catch (Exception ex)
            {
            ...
            dt_teclado.DataSource = dt;
            dt_teclado.Columns["id"].Visible = false;
            dt_teclado.Columns["AulaID"].Visible = false;
        }
        private void crearTeclado()
        {
            try
            {
                ETeclado teclado = new ETeclado
                {
                    Cbi = txt_cbi.Text,
                ...
                int resultado = control.Create(teclado);
        ...
        private void select(DataGridViewCellEventArgs e)
        {
            ...
            var fila = dt_teclado.Rows[indice];
        ...
        private void dt_teclado_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            select(e);
        }
        ...
        private void dt_teclado_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
